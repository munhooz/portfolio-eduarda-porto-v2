...
const createCategoryId = () => `vitrine-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

const normalizeCategories = (categories: SiteData["projectSubsections"]["vitrine"]["categories"]) =>
...
const VitrineSection = ({ data, onChange }: VitrineSectionProps) => {
  ...
  const categories = [...data.projectSubsections.vitrine.categories].sort((a, b) => a.ordem - b.ordem);

  const updateVitrine = (nextVitrine: SiteData["projectSubsections"]["vitrine"]) => {
    onChange({
      ...data,
      projectSubsections: {
  ...
  const updateCategory = (
    categoryId: string,
    changes: Partial<SiteData["projectSubsections"]["vitrine"]["categories"][number]>,
  ) => {
    updateVitrine({
      ...data.projectSubsections.vitrine,
  ...
  const addCategory = () => {
    const newCategory: SiteData["projectSubsections"]["vitrine"]["categories"][number] = {
      id: createCategoryId(),
      titulo: "",
      imagens: [],
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-muted-foreground">
        Edite os textos da página /vitrine, crie novas categorias e organize a ordem de exibição com drag and drop.
      </p>
      ...
      <div className="space-y-4 rounded-xl border border-border/70 bg-muted/15 p-4">
        <div>
          ...
          <input
            ...
            value={data.projectSubsections.vitrine.pageTitle}
            onChange={(e) =>
              updateVitrine({
                ...data.projectSubsections.vitrine,
        ...
        <div>
          ...
          <textarea
            className={inputClass}
            rows={3}
            value={data.projectSubsections.vitrine.pageSubtitle}
            onChange={(e) =>
              updateVitrine({
                ...data.projectSubsections.vitrine,
                    ...
                    <div className="flex items-center gap-3 p-4 hover:bg-muted/30 transition-colors">
                      ...
                      <div className="ml-3 flex items-center gap-2">
                        <ConfirmDeleteButton
                          ...
                          description="A categoria e todas as imagens dentro dela serão removidas da vitrine."
                    ...
                    {isExpanded && (
                      <div className="space-y-3 border-t border-border p-4 pt-3">
                        ...
                        <ImagesListEditor
                          ...
                          folder={`eduarda-porto/vitrine/${category.id}`}
                        />
                      </div>
                    )}
                  </div>
