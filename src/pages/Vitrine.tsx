...
const Vitrine = () => {
  ...
  const vitrineProject = useMemo(
    () => data.projects.find((project) => project.link === "/vitrine"),
    [data.projects],
  );

  const categories = useMemo(
    () =>
      [...data.projectSubsections.vitrine.categories]
    ...
    <div className="min-h-screen bg-background">
      <div className="section-padding pb-8 pt-12">
        <div className="mx-auto max-w-7xl">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <h1 className="mb-4 text-4xl font-bold md:text-5xl">{data.projectSubsections.vitrine.pageTitle}</h1>
            <p className="max-w-xl text-muted-foreground">{data.projectSubsections.vitrine.pageSubtitle}</p>
          </motion.div>
        </div>
      </div>
      ...
      {categories.length === 0 ? (
        <section className="section-padding pt-0">
          <div className="mx-auto max-w-7xl">
            <p className="text-sm text-muted-foreground">Nenhuma categoria cadastrada para a vitrine.</p>
          </div>
        </section>
      ) : (
                  ...
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                    {category.imagensFormatadas.map((img, imageIndex) => (
                      <motion.div
                        ...
                        className="group cursor-pointer overflow-hidden rounded-xl glass-card transition-all duration-300 hover:shadow-lg"
